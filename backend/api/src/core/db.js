...
if (!process.env.DB_DATABASE_TEST && process.env.TEST === "true") {
  throw Error("Не указана тестовая база данных в env");
}
...
sequelize.query = async function () {
  try {
    ...
    const error = new DatabaseError(null, err);
    throw error;
  }
};
